//This program subtracts notes
//z5309196 Christopher Luong 07/04/2020
...
Note note_subtract(Note higher, Note lower) {
    Note result = malloc(sizeof(struct note));
    int keyDiff = 0;
    int octDiff = 0;

    keyDiff = higher->key - lower->key;
    if (keyDiff < 0) {
        keyDiff = 12 + keyDiff;
        octDiff = higher->octave - lower->octave - 1;
    } else {
        octDiff = higher->octave - lower->octave;
    }
    result->octave = octDiff; 
    result->key = keyDiff; 
    result->next = NULL;
    return result;
