//This program deletes a certain value
//from a linked list
//z5309196 Christopher Luong 14/04/2020
...
struct node *delete_contains(int value, struct node *head) {
    ...
    if (head == NULL) {
        return head;
    }//keep looping until we find the matching value
    while (curr != NULL && curr->data != value) {
        prev = curr;
        curr = curr->next;
    }//if current isn't NULL, value has been found
    if (curr != NULL) {
        //it's the first node
        if (prev == NULL) {
            head = curr->next;
        ...
        free(curr);
    }//if current is NULL, list doesn't change    
